...
def recursive_comparison(current_node: TreeNode, key: int, tree_comparisons: int):
    if current_node.key == key:
        tree_comparisons += 1
        return tree_comparisons
    if current_node.key != key:
        tree_comparisons += 1
    elif key > current_node.key:
        tree_comparisons += 1
        recursive_comparison(current_node.right, key, tree_comparisons)
    elif key < current_node.key:
        tree_comparisons += 1
        recursive_comparison(current_node.left, key, tree_comparisons)
...
class BinarySearchTree:
    ...
    def find_comparison(self, key: int) -> Tuple[int, int]:
        ...
        tree_comparisons = 0
        print(recursive_comparison(self._root, key, tree_comparisons))
        self._num_of_comparisons = tree_comparisons
